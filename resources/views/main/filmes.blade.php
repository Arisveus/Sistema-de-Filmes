        ...
        <div
            ...
            @if($filme->imagem)
                @if(Str::startsWith($filme->imagem, 'http'))
                    <img src="{{ $filme->imagem }}" alt="{{ $filme->nome }}" class="w-full h-48 object-cover mb-4 rounded">
                @else
            ...
            <form action="{{ route('lista.store', $filme->id) }}" method="POST">
                @csrf
                <button type="submit" class="bg-green-600 hover:bg-green-700 text-black font-semibold py-1 px-3 rounded shadow">Adicionar na Lista de Filmes</button>
            </form>
        </div>
        @endforeach
    </div>
