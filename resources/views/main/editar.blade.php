...
@section('content')
<div class="container-add editar mx-auto py-8 max-w-lg">
    <h1 class="titulo-add text-2xl font-bold mb-6">Editar Filme</h1>
    <form action="{{ route('filmes.update', $filme->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="nome" id="nome" value="{{ $filme->nome }}" class="input-color w-full border rounded px-3 py-2" required>
        </div>
        <div>
            <label for="sinopse" class="block font-semibold">Sinopse</label>
            <textarea name="sinopse" id="sinopse" rows="3" class="input-color w-full border rounded px-3 py-2 resize-none" required>{{ $filme->sinopse }}</textarea>
        </div>
        <div>
            <label for="ano" class="block font-semibold">Ano</label>
            <input type="date" name="ano" id="ano" value="{{ $filme->ano }}" class="input-color w-full border rounded px-3 py-2" required>
        </div>
        <div>
        ...
        <div>
            ...
            <input type="url" name="trailer" id="trailer" value="{{ $filme->trailer }}" class="input-color w-full border rounded px-3 py-2" placeholder="https://youtube.com/...">
        </div>
        <div>
        ...
        <button type="submit" class="btn-save text-white font-semibold py-2 px-4 shadow">Salvar Alterações</button>
    </form>
</div>
@endsection
